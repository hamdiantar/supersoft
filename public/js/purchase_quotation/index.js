function quotationType() {
    ...
    if (quotation_type == 'from_purchase_request') {
...
function calculateTotal() {
    ...
    for (let i = 1; i <= items_count; i++) {
        ...
        if ($('#checkbox_item_' + i).is(':checked')) {
...
function calculateAdditionalPayments() {
    ...
    // var total = parseFloat(total_after_discount) + parseFloat(total_additional);
    //
    // $("#total").val(total.toFixed(2));
...
function checkAllItems () {
    ...
    if ($('.select_all_items').is(':checked')){
        ...
        $('.item_of_all').prop('checked', true);
    }else {

        $('.item_of_all').prop('checked', false);
    }
}
