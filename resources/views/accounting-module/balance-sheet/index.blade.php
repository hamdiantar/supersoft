...
@section('title')
    <title>{{ __('Super Car') }} - {{ __('accounting-module.balance-sheet-index') }} </title>
@endsection
...
@section('content')
    <nav>
        ...
        <li class="breadcrumb-item"><a href="{{route('admin:home')}}"> {{__('Dashboard')}}</a></li>
            <li class="breadcrumb-item active"> {{ __('accounting-module.balance-sheet-index') }} </li>
        </ol>
    </nav>
...
@section('accounting-scripts')
    ...
    <script type="application/javascript">
        $(document).ready(function() {
            @if($totals->balance < 0)
                alert('{{ __('accounting-module.balance-sheet-less').' '. abs($totals->balance) }}')
            @elseif($totals->balance > 0)
                alert('{{ __('accounting-module.balance-sheet-great').' '. abs($totals->balance) }}')
            @endif
        })
    </script>
