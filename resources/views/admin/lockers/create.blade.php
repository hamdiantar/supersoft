...
@section('title')
<title>{{ __('Super Car') }} - {{ __('Create Locker') }} </title>
@endsection
...
@section('content')
    ...
    <nav>
            ...
            <li class="breadcrumb-item">  {{__('Create Locker')}} </li>
            </ol>
        </nav>
        ...
        <div class=" card box-content-wg-new bordered-all primary">
                <h4 class="box-title with-control" style="text-align: initial"><i class="fa fa-folder-open-o"></i>  {{__('Create Locker')}}
                ...
                <button class="control text-white" style="background:none;border:none;font-size:12px">{{__('Save')}}<img class="img-fluid" style="width:50px;height:50px;margin-top:-20px;margin-bottom:-13px" src="{{asset('assets/images/f1.png')}}"></button>
                    ...
                    <form method="post" action="{{route('admin:lockers.store')}}" class="form">
                        @csrf
                        @method('post')
...
@section('js-validation')
    {!! JsValidator::formRequest('App\Http\Requests\Admin\Lockers\CreateLockersRequest', '.form'); !!}
