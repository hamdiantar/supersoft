    ...
    <div style="border: #AAA solid 1px;border-top:20px solid grey;border-radius:15px 15px 5px 5px;min-height:170px">
        ...
        <div class="num-w">
            <div style="border: #AAA solid 1px;border-radius:5px;padding:10px;margin-top:10px">
                <h3 class="text-center" style="font-weight:bold ">{{ __('words.bank-receive') }}</h3>
                    ...
                    <div class="col-xs-6">
                        <table class="table table-bordered">
                            <tbody>
                                ...
                                @if($receive->source_type == 'bank')
                                    <td>{{optional($receive->exchange_permission->fromBank)->name}}</td>
                                @else
                                    <td>{{optional($receive->locker_exchange_permission->fromLocker)->name}}</td>
                                @endif
                            </tbody>
                        </table>
                    </div>
                    <div class="col-xs-6">
                        <table class="table table-bordered">
                            <tbody>
                                ...
                                @if($receive->source_type == 'bank')
                                    <td>{{optional($receive->exchange_permission->toBank)->name}}</td>
                                @else
                                    <td>{{optional($receive->locker_exchange_permission->toBank)->name}}</td>
                                @endif
                            </tbody>
                        </table>
                    </div>
...
<div>
    <div style="margin-top:13px">
        @if($receive->status == 'pending')
            <a href="{{ route('admin:bank-receives.approve' ,['id' => $receive->id]) }}" class="btn btn-success">
                <i class="fa fa-check"></i> {{ __('words.approve-permission') }}
            </a>
            <a href="{{ route('admin:bank-receives.reject' ,['id' => $receive->id]) }}" class="btn btn-warning">
