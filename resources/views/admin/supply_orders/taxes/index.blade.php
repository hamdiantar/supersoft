<div class="remodal" data-remodal-id="m-2" role="dialog"
    ...
    <button data-remodal-action="close" class="remodal-close" aria-label="Close"></button>
        ...
        <h2 class="box-title with-control" id="modal1Title">{{__('Taxes')}}</h2>
        ...
        <div class="form-group has-feedback col-sm-12">
            <table class="table table-bordered" style="width:100%">
                ...
                @foreach($taxes as $index=>$tax)
                    <tr>
                        <td>
                            <input type="checkbox" id="invoie_checkbox_tax_{{$index}}" name="items[{{$index}}][taxes][]"
                                   data-tax-value="{{$tax->value}}" data-tax-type="{{$tax->tax_type}}" onclick="calculateItem('{{$index}}')"
                            >
                        </td>
                        <td>
                ...
                @endforeach
                <input type="hidden" name="tax_count" id="invoice_tax_count" value="{{$taxes->count()}}">
                </tbody>
            </table>
