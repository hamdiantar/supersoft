...
<thead>
    <tr>
        ...
        <th class="text-center column-locker-from" 
            onclick="appling_sort(event ,'locker-from')"
            data-sort-method="{{ isset($_GET['sort_method']) ? $_GET['sort_method'] : 'desc' }}"
            scope="col">
            {!! __('Locker From') !!}
            <i class="fa fa-sort{{ \App\AccountingModule\Helper::iam_used_in_sort('locker-from') }}"></i>
        </th>
        <th class="text-center column-locker-to" 
            onclick="appling_sort(event ,'locker-to')"
            data-sort-method="{{ isset($_GET['sort_method']) ? $_GET['sort_method'] : 'desc' }}"
            scope="col">
            {!! __('Locker To') !!}
            <i class="fa fa-sort{{ \App\AccountingModule\Helper::iam_used_in_sort('locker-to') }}"></i>
        </th>
