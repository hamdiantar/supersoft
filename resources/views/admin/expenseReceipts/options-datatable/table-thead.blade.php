<thead>
    <tr>
        <th class="text-center column-expense-no" 
            onclick="appling_sort(event ,'expense-no')"
            data-sort-method="{{ isset($_GET['sort_method']) ? $_GET['sort_method'] : 'desc' }}"
            scope="col">
            {!! __('Expense No') !!}
            <i class="fa fa-sort{{ \App\AccountingModule\Helper::iam_used_in_sort('expense-no') }}"></i>
        </th>
        ...
        <th class="text-center column-expense-item" 
            onclick="appling_sort(event ,'expense-item')"
            data-sort-method="{{ isset($_GET['sort_method']) ? $_GET['sort_method'] : 'desc' }}"
            scope="col">
            {!! __('Expense Item') !!}
            <i class="fa fa-sort{{ \App\AccountingModule\Helper::iam_used_in_sort('expense-item') }}"></i>
        </th>
