...
<td>
    <div class="radio primary">
        ...
        {{$winchType && $winchType->cardInvoiceWinchRequest ? 'checked':''}}
        >
    </div>
</td>
...
<td>
    <input type="text" class="form-control" disabled id="branch_distance"
           value="{{$winchType && $winchType->cardInvoiceWinchRequest ? $winchType->cardInvoiceWinchRequest->distance:''}}">
</td>
...
<td>
    <div class="radio primary">
        <input type="radio" name="winch_discount_type" id="winch-discount-type-amount" value="amount" checked onclick="winchDiscount()"
               {{!$winchType ? 'checked': ''}}
            {{ $winchType && $winchType->cardInvoiceWinchRequest->discount_type == 'amount' ? 'checked':''}}
        >
    ...
    <div class="radio primary">
        <input type="radio" name="winch_discount_type" id="winch-discount-type-percent" value="percent" onclick="winchDiscount()"
            {{$winchType && $winchType->cardInvoiceWinchRequest->discount_type == 'percent' ? 'checked':''}}>
...
<td>
    <input  type="number" class="form-control" name="winch_discount" id="winch-discount"
            ...
            value="{{$winchType && $winchType->cardInvoiceWinchRequest ? $winchType->cardInvoiceWinchRequest->discount: 0}}"
    >
</td>

<td>
    <input  type="number" class="form-control" readonly name="winch_total_before_discount" id="winch-total"
            value="{{$winchType && $winchType->cardInvoiceWinchRequest ? $winchType->cardInvoiceWinchRequest->sub_total:0}}">
</td>

<td>
    <input type="number" class="form-control" readonly name=winch_total_after_discount" id="winch-total-after-discount"

           value="{{$winchType && $winchType->cardInvoiceWinchRequest ? $winchType->cardInvoiceWinchRequest->total:0}}"
    >
</td>
