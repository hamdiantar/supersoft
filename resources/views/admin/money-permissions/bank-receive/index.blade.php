...
@section('title')
    <title>{{ __('Super Car') }} - {{__('words.bank-receives')}} </title>
@endsection
    ...
    <div class="row small-spacing">
        <nav>
            <ol class="breadcrumb" style="font-size: 37px; margin-bottom: 0px !important;padding:0px">
                ...
                <li class="breadcrumb-item">  {{__('words.bank-receives')}}</li>
            </ol>
        </nav>
            ...
            <div class="box-content card bordered-all js__card">
                ...
                <div class="card-content js__card_content" style="">
                    <ul class="list-inline pull-left top-margin-wg">
                        <li class="list-inline-item">
                            @include('admin.buttons.add-new', [
                                'route' => 'admin:bank-receives.create',
                                'new' => '',
                            ])
                        </li>
                        <li class="list-inline-item">
                            @component('admin.buttons._confirm_delete_selected',[
                                'route' => 'admin:bank-receives.delete_selected',
                            ])
                            @endcomponent
                        </li>
                    </ul>
                    ...
                    <div class="table-responsive">
                        @php
                            $view_path = 'admin.money-permissions.bank-receive.options-datatable';
                        @endphp
                                ...
                                <tr>
                                    ...
                                    <td class="text-center column-exchange-number">
                                        @if($exchange->source_type == 'locker')
                                            {!! optional($exchange->locker_exchange_permission)->permission_number !!}
                                        @else
                                    ...
                                    <td>
                                        @if($exchange->status == 'pending')
                                            @component('admin.buttons._edit_button',[
                                                'id' => $exchange->id,
                                                'route' => 'admin:bank-receives.edit',
                                            ])
                                            @endcomponent

                                            @component('admin.buttons._delete_button',[
                                                'id' => $exchange->id,
                                                'route' => 'admin:bank-receives.destroy',
                                                'tooltip' => __('Delete '.$exchange->permission_number),
                                            ])
                                            @endcomponent
                                        @endif
                                        <a class="btn btn-info"
                                            onclick="load_money_permission_model('{{ route('admin:bank-receives.show' ,['id' => $exchange->id]) }}')">
