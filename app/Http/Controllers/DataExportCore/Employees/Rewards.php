...
class Rewards {
    ...
    function __construct($query_builder ,$visible_columns) {
        ...
        $this->columns = [
            'name' ,'type', 'cost', 'date', 'created_at', 'updated_at'
        ];
        $this->header = [
    ...
    private function print() {
        ...
        return view('server-side-print' ,[
            ...
            'header' => $this->header,
            'title' => __('Employees Rewards Printing'),
                    ...
                    foreach ($columns_keys as $key) {
                        switch ($key) {
                            case 'name':
                                ...
                                break;
                            case 'type':
                                $value = $record->type === __('reward') ? __('Reward') : __('The Discount');
                                break;
                            case 'cost':
                                $value = number_format($record->cost, 2);
                                break;
                            default:
                                $value = $record[$key];
                                break;
    ...
    private function excel() {
        ...
        return Excel::download($export_object, 'Employees-Rewards-'.date('YmdHis').'.csv');
    }
}