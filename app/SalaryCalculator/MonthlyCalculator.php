...
class MonthlyCalculator extends CoreCalculator {
    private $dailyWorkHour ,$workDayMoney ,$workHourMoney ,$actual_month_days ,$work_month ,$work_year;
    ...
    function __construct($employee, $date_range) {
        ...
        $this->workDayMoney = $this->employee->employeeSetting->amount_account / 30;//$this->actual_month_days;

        $this->workHourMoney = $this->workDayMoney / $this->dailyWorkHour;
        // $this->employee->employeeSetting->amount_account / ($this->dailyWorkHour * $this->actual_work_days());

        $this->set_additional_hour_value();
        $this->set_delay_hour_value();
        $this->set_absence_day_value();

        $this->moth_salary = $this->employee->employeeSetting->amount_account;
    }

    private function set_actual_month_days() {
        $start = new Carbon($this->date_range[0]);
        $this->actual_month_days = $start->daysInMonth;
        $this->work_month = $start->month;
        $this->work_year = $start->year;
    }
    ...
    private function actual_work_days() {
        $actual_work_days = 0;

        $start = new Carbon($this->work_year . '-' . $this->work_month . '-1');
        $end = new Carbon($this->work_year . '-' . $this->work_month . '-' . $this->actual_month_days);
        
        while($start <= $end) {
            $dayKey = strtolower(substr($start->format('l') ,0 ,3));
            if (!in_array($dayKey , $this->vacations)) $actual_work_days++;
            $start = $start->addDay();
        }
