...
class LockerReceives {
    ...
    private function print() {
        ...
        return view('server-side-print' ,[
            ...
            'header' => $this->header,
            'title' => __('words.locker-receives'),
            'custom_echo' => function ($columns_keys ,$record) {
                echo '<tr>';
                    foreach ($columns_keys as $key) {
                        switch ($key) {
                            case 'exchange_number':
                                $value = $record->source_type == 'locker' ? optional($record->exchange_permission)->permission_number : optional($record->bank_exchange_permission)->permission_number;
                                break;
                            case 'source_type':
    ...
    private function excel() {
        ...
        return Excel::download($export_object, 'Locker-Receives-'.date('YmdHis').'.csv');
    }
}