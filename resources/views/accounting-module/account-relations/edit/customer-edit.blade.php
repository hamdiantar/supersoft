<form id="account-relations-customers-form" method="post" action="{{ $form_route }}">
    @csrf
    <input type="hidden" name="action_for" value="customers"/>
    ...
    <div class="form-group col-md-8">
        ...
        <div class="form-check">
            <input class="form-check-input" type="radio" id="global" value="global"
                name="related_as" {{ $model->related_actor->related_as == 'global' ? 'checked' : '' }}>
    ...
    <div class="form-group col-md-4" id="select-actor_id" style="display: {{ $model->related_actor->related_as == 'actor_id' ? 'block' : 'none' }}">
        ...
        <select name="customer_id" class="form-control select2" style="width:100%">
            ...
            @foreach($customers as $customer)
                <option {{ $model->related_actor->related_as == 'actor_id' && $model->related_actor->related_id == $customer->id ? 'selected' : '' }}
                    value="{{$customer->id}}">
                    {{ $customer->name }}
                </option>
            @endforeach
