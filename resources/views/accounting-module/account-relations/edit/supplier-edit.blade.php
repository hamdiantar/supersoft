<form id="account-relations-suppliers-form" method="post" action="{{ $form_route }}">
    @csrf
    <input type="hidden" name="action_for" value="suppliers"/>
    ...
    <div class="form-group col-md-4" id="select-actor_id" style="display: {{ $model->related_actor->related_as == 'actor_id' ? 'block' : 'none' }}">
        ...
        <select name="supplier_id" class="form-control select2" style="width:100%">
            ...
            @foreach($suppliers as $supplier)
                <option {{ $model->related_actor->related_as == 'actor_id' && $model->related_actor->related_id == $supplier->id ? 'selected' : '' }}
