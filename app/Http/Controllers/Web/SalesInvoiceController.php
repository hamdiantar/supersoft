...
use App\Http\Controllers\DataExportCore\CustomerPanel\Sales;
...
class SalesInvoiceController extends Controller
{
    public function index(Request $request)
    {
        ...
        $invoices = SalesInvoice::query()->where('customer_id', $auth->id);
        ...
        $salesInvoices = SalesInvoice::where('customer_id', $auth->id)->select('id','invoice_number')->get();

        return view('web.sales_invoices.index', compact('invoices','salesInvoices'));
    }
    ...
    public function show(Request $request)
    {
        ...
        $setting = Setting::where('branch_id', $auth->branch_id)->where('sales_invoice_status', 1)->first();

        $invoice = view('web.sales_invoices.show',
