...
@section('title')
<title>{{ __('Super Car') }} - {{__('Edit Account')}} </title>
@endsection
...
@section('content')
    ...
    <nav>
            ...
            <li class="breadcrumb-item">  {{__('Edit Account')}} </li>
            </ol>
        </nav>
        ...
        <div class=" card box-content-wg-new bordered-all primary">
                <h4 class="box-title with-control" style="text-align: initial"><i class="fa fa-money"></i>  {{__('Edit Account')}}
                ...
                <button class="control text-white" style="background:none;border:none;font-size:12px">{{__('Save')}}<img class="img-fluid" style="width:50px;height:50px;margin-top:-20px;margin-bottom:-13px" src="{{asset('assets/images/f1.png')}}"></button>
                    ...
                    <form method="post" action="{{route('admin:accounts.update',$account->id)}}" class="form"
...
@section('js-validation')
        {!! JsValidator::formRequest('App\Http\Requests\Admin\Accounts\UpdateAccountsRequest', '.form'); !!}
