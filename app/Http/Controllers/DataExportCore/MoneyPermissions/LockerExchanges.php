...
class LockerExchanges {
    ...
    private function print() {
        ...
        return view('server-side-print' ,[
            ...
            'header' => $this->header,
            'title' => __('words.locker-exchanges'),
            'custom_echo' => function ($columns_keys ,$record) {
                echo '<tr>';
                    foreach ($columns_keys as $key) {
                        switch ($key) {
                            case 'locker_from':
                                $value = optional($record->fromLocker)->name;
                                break;
                            case 'locker_to':
                                $value = $record->destination_type == 'bank' ? optional($record->toBank)->name : optional($record->toLocker)->name;
                                break;
    ...
    private function excel() {
        ...
        return Excel::download($export_object, 'Locker-Exchanges-'.date('YmdHis').'.csv');
    }
}