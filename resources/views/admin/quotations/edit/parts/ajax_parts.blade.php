                                                           ...
                                                           href="#" id="part_details">
                                                            <div class="card-title">
                                                                ...
                                                                <h2 class="text-center"
                                                                    style="font-size: 12px; display: none;">{{$part->barcode}}
                                                                    <i class="fa fa-plus"></i>
                                                                </h2>
                                                            </div>
                                                        </a>
            ...
            <div class="form-group has-feedback">
                <div class="anyClass-wg table-responsive">
                    <table id="invoiceItemsDetails" class="table table-bordered" style="width:100%">
                        @include('admin.quotations.parts.table-footer')
                        ...
                        @foreach($partType->items as $index=>$type_item)
                            @php
                                $part = $type_item->part;
                                $purchaseInvoice = $type_item->purchaseInvoice;
                                $invoice_item =  $purchaseInvoice->items()->where('part_id', $part->id)->first();
                                 $index += 1;
                            @endphp

                            <tr data-id="{{$index}}" id="remove_part_{{$index}}" class="tr_{{$index}}">
                                @include('admin.quotations.edit.parts.purchase_invoice_data')
                                <input type="hidden" name="part_id" value="{{$index}}" id="part-{{$index}}">
                            </tr>
                        @endforeach
                        </tbody>

                    </table>
                    <input type="hidden" name="parts_items_count" id="parts_items_count" value="{{$partType->items->count()}}">
                </div>
            </div>
