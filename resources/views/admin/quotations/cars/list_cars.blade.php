...
<div class="box-content box-content-wg card bordered-all blue-1 js__card">
    ...
    <button type="button" class="btn hvr-rectangle-in closeAdd-wg-btn  " data-dismiss="modal"><i class="ico ico-left fa fa-close"></i>رجوع</button>
</div>
@section('js-car')
    <script type="application/javascript">
            ...
            var table = $('#cars').DataTable({
                ...
                serverSide: true,
                ajax: {
                    "url": "{{ route('admin:cars.modal', ['id' => 0])}}",
                    "data": function ( d ) {
                        return $.extend( {}, d, {
                            "customer_id": $('.customerIDFromController').val()
                        } );
                    }
                },
                columns: [
            ...
            function fillCarForm(carData)
            {
                ...
                <option value="{{$type->id}}" {{$type->id ? 'selected' : ''}}>{{$type->type}}</option>
                @endforeach`;

                let companies =  `<option value="">{{__('Select Company')}}</option>
                @foreach(\App\Models\Company::all() as $company)
                <option value="{{$company->id}}" {{$company->id ? 'selected' : ''}}>{{$company->name}}</option>
                @endforeach`;

                let models =  `<option value="">{{__('Select Car Model')}}</option>
                @foreach(\App\Models\CarModel::all() as $carModel)
                <option value="{{$carModel->id}}" {{$carModel->id ? 'selected' : ''}}>{{$carModel->name}}</option>
                @endforeach`;
