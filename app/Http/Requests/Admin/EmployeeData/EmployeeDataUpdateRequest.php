...
class EmployeeDataUpdateRequest extends FormRequest
{
    ...
    {
        $empData = request()->segment(4);
        $id = EmployeeData::find($empData)->id;
       $rules = [
            'name_ar'=>'required|string|max:255',
            'name_en'=>'required|string|max:255',
            'country_id' =>'nullable|exists:countries,id',
            'employee_setting_id' =>'integer|exists:employee_settings,id',
            'city_id' =>'nullable|exists:cities,id',
            'area_id' =>'nullable|exists:areas,id',
            'cv' => 'mimes:pdf,doc,docx|max:2048',
            'start_date_assign' => 'required',
            'email' => 'nullable|email|max:128|unique:employee_data,email,'.$id
        ];
