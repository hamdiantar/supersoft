...
@section('title')
    <title>{{ __('Super Car') }} - {{ __('Edit Employees Attendance / Departure') }} </title>
@endsection
...
@section('content')
    ...
    <nav>
            <ol class="breadcrumb" style="font-size: 37px; margin-bottom: 0px !important;padding:0px">
                ...
                <li class="breadcrumb-item active"> {{ __('Edit Employees Attendance / Departure') }}</li>
            </ol>
        </nav>
        <div class="col-xs-12">
        <div class=" card box-content-wg-new bordered-all primary">
                <h4 class="box-title with-control" style="text-align: initial"><i class="fa fa-user"></i>  {{ __('Edit Employees Attendance / Departure') }}
                ...
                <button class="control text-white" style="background:none;border:none;font-size:12px">{{__('Save')}}<img class="img-fluid" style="width:50px;height:50px;margin-top:-20px;margin-bottom:-13px" src="{{asset('assets/images/f1.png')}}"></button>
                    ...
                    <form method="post" action="{{route('admin:employees_attendance.update', ['id' => $employeeAttendance->id])}}" class="form">
                        @csrf
                        @method('put')
                        <div class="row">
                        @if (authIsSuperAdmin())
                                            ...
                                            <div class="input-group">
                                                ...
                                                <select name="branch_id" class="form-control js-example-basic-single" onchange="getEmpByBranch(event)">
                                                    ...
                                                    @foreach(\App\Models\Branch::all() as $branch)
                                                        ...
                                                        {{$employeeAttendance->branch_id === $branch->id ? 'selected' : ''}}>
                                                            {{$branch->name}}</option>
                                                    @endforeach
                                                </select>
                            ...
                            <div class="col-md-12">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label> {{__('Select Employee Name')}} <i class="req-star" style="color: red">*</i></label>
                                        <div class="input-group">
                                            ...
                                            <select name="employee_data_id" class="form-control js-example-basic-single" id="setEmpByBranch">
                                                ...
                                                @foreach(\App\Models\EmployeeData::all() as $emp)
                                                    <option value="{{$emp->id}}"
                                                        {{$employeeAttendance->employee_data_id === $emp->id ? 'selected' : ''}}>
                                                        {{$emp->name}}</option>
                                                @endforeach
                                            </select>
                                        ...
                                        <ul class="list-inline">
                                            <li>
                                                <div class="radio info">
                                                    <input type="radio" name="type" value="attendance"
                                                           {{$employeeAttendance->type === __('attendance') ? 'checked' : ''}}
                                                           id="radio-10"><label
                                            ...
                                            <li>
                                                <div class="radio pink">
                                                    <input type="radio" name="type" value="departure"
                                                           {{$employeeAttendance->type === __('departure') ? 'checked' : ''}}
                                                           id="radio-12"><label
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="date" class="form-control time" name="date" value="{{$employeeAttendance->date}}" >
                                    </div>
                                </div>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="time" class="form-control time" name="time" value="{{$employeeAttendance->time}}" >
                                    </div>
                                </div>
                            </div>
                        </div>
