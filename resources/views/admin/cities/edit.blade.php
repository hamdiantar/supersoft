    ...
    <div class="row small-spacing">
        ...
        <div class=" card box-content-wg-new bordered-all primary">
            ...
            <div class="box-content">
                <form method="post" action="{{route('admin:cities.update', ['id' => $city->id])}}" class="form">
                    @csrf
                    @method('put')
                    ...
                    <div class="col-md-12">
                        ...
                        <span class="input-group-addon"><i class="fa fa-file-o"></i></span>
                            <input type="text" name="name_ar" class="form-control" value="{{$city->name_ar}}" id="inputNameAR" placeholder="{{__('Name In arabic')}}">
                        </div>
                        ...
                        <div class="input-group">
                        <span class="input-group-addon"><i class="fa fa-file-o"></i></span>
                            <input type="text" name="name_en" class="form-control" id="inputNameEN" value="{{$city->name_en}}" placeholder="{{__('Name In English')}}">
                        </div>
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="col-md-6">
                        <div class="form-group has-feedback">
                        <label for="inputSymbolAR" class="control-label">{{__('Select Country')}}</label>
                        <div class="input-group">
                                <span class="input-group-addon fa fa-globe"></span>
                        <select name="country_id" class="form-control js-example-basic-single">
                            @foreach(\App\Models\Country::all() as $county)
                                <option value="{{$county->id}}" {{$city->country_id == $county->id ? 'selected' : ''}}>{{$county->name}}</option>
                            @endforeach
                        </select>
...
@section('js-validation')
    {!! JsValidator::formRequest('App\Http\Requests\Admin\City\CityRequest', '.form'); !!}
