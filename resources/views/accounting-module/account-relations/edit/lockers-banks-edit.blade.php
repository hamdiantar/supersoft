<form id="account-relations-lockers-banks-form" method="post" action="{{ $form_route }}">
    ...
    <div class="form-group col-md-6">
        ...
        <div class="form-check">
            <input class="form-check-input" type="radio" name="account_nature" id="locker" value="locker"
                {{ $model->related_locker_bank->related_as == 'locker' ? 'checked' : '' }}>
            ...
            <input class="form-check-input" type="radio" name="account_nature" id="bank_acc" value="bank_acc"
                {{ $model->related_locker_bank->related_as == 'bank' ? 'checked' : '' }}>
            <label class="form-check-label" for="bank_acc">
                {{ __('accounting-module.bank_acc') }}
            </label>
        </div>
    </div>
    <div class="form-group col-md-6" id="locker-id" style="display: {{ $model->related_locker_bank->related_as == 'locker' ? 'block' : 'none' }}">
        ...
        <select name="locker_id" class="form-control select2" style="width:100%">
            ...
            @foreach(\App\Models\Locker::where('status',1)->where('branch_id', auth()->user()->branch_id)->get() as $locker)
                <option value="{{$locker->id}}"
                    {{ $model->related_locker_bank->related_as == 'locker' && $model->related_locker_bank->related_id == $locker->id ? 'selected' : '' }}>
                    {{ $locker->get_trans_name() }}
                </option>
            @endforeach
        </select>
    </div>
    <div class="form-group col-md-6" id="bank-acc-id" style="display: {{ $model->related_locker_bank->related_as == 'bank' ? 'block' : 'none' }}">
        ...
        <select name="bank_id" class="form-control select2" style="width:100%">
            ...
            @foreach(\App\Models\Account::where('status',1)->where('branch_id', auth()->user()->branch_id)->get() as $account)
                <option value="{{$account->id}}"
                    {{ $model->related_locker_bank->related_as == 'bank' && $model->related_locker_bank->related_id == $account->id ? 'selected' : '' }}>
                    {{ $account->get_trans_name() }}
                </option>
            @endforeach
        </select>
    </div>
