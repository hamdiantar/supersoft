<form id="account-relations-edit-form" method="post" action="{{ $form_route }}">
    @csrf
    ...
    <div class="form-group col-md-12">
        ...
        <div class="form-check">
            <input class="form-check-input" type="radio" name="account_nature" id="credit"
                {{ $model->related_expense_revenue->related_as == 'credit' ? 'checked' : '' }} value="credit">
            <label class="form-check-label" for="credit">
                {{ __('accounting-module.credit') }}
            </label>
            <input class="form-check-input" type="radio" name="account_nature" id="debit"
                {{ $model->related_expense_revenue->related_as == 'debit' ? 'checked' : '' }} value="debit">
    ...
    <div class="form-group col-md-6" id="accounts-tree-root-id">
        ...
        <select name="accounts_tree_root_id" class="form-control select2" style="width:100%">
            ...
            @foreach($root_accounts_tree as $root_acc)
                <option {{ $root_acc->id == $model->accounts_tree_root_id ? 'selected' : '' }}
                    value="{{ $root_acc->id }}">
    ...
    <div class="form-group col-md-12" id="save-btn">
        <button id="save-btn-clickable" type="button" class="btn hvr-rectangle-in saveAdd-wg-btn"><i class="ico ico-left fa fa-save"></i> {{ __('words.save') }} </button>
