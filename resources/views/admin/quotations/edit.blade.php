...
@section('title')
    <title>{{ __('Super Car') }} - {{ __('Edit Quotation') }} </title>
@endsection
        ...
        <nav>
            <ol class="breadcrumb" style="font-size: 37px; margin-bottom: 0px !important;padding:0px">
                ...
                <li class="breadcrumb-item"><a
                            href="{{route('admin:quotations.index')}}"> {{__('Quotations')}}</a></li>
                <li class="breadcrumb-item active"> {{__('Edit Quotation')}}</li>
            </ol>
        </nav>
        ...
        <div class="col-xs-12">
            <div class=" card box-content-wg-new bordered-all primary">
                ...
                <div class="box-content">
                    <form id="quotation_forms" action="{{route('admin:quotations.update', $quotation->id)}}"
                          class="form" method="post">
                        @csrf
                        @method('post')

                        @include('admin.quotations.edit.info_section')

                        <div class="form-group has-feedback col-md-3" id="customer_data" style="padding-bottom: 19px;">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-3">
                                <div class="form-group has-feedback">
                                    ...
                                    <input type="checkbox" id="spar_parts" name="parts_box"
                                           {{isset($quotation) && $partType ? 'checked' : ''}}
                                           onclick="getPartsSection(); showTotalDetails()">
                            ...
                            <div class="col-md-3">
                                <div class="form-group has-feedback">
                                    ...
                                    <input type="checkbox" id="services_section_checkbox"
                                           {{isset($quotation) && $quotationServiceType ? 'checked' : ''}}
                                           name="services_box"
                            ...
                            <div class="col-md-3">
                                <div class="form-group has-feedback">
                                    ...
                                    <input type="checkbox" id="package_section_checkbox"
                                           {{isset($quotation) && $packageType ? 'checked' : ''}}
                                           name="packages_box"
                                           onclick="getPackagesSection(); showTotalDetails()">

                                    <img src="{{asset('default-images/loading.gif')}}" id="packages_loading"
                                         style=" height: 32px; width: 32px;margin-top: -11px; display: none;">
                                </div>
                            </div>
                            ...
                            <div class="col-md-3">
                                <div class="form-group has-feedback">
                                    ...
                                    <input type="checkbox" id="winch_section_checkbox" name="winch_box"
                                           {{isset($quotation) && $winchType ? 'checked' : ''}}
                                           onclick="getWinchSection(); showTotalDetails()">

                                    <img src="{{asset('default-images/loading.gif')}}" id="winch_loading" style=" height: 32px; width: 32px;margin-top: -11px; display: none;">
                                </div>
                            </div>

                        @include('admin.quotations.edit.parts.spareParts')
                        @include('admin.quotations.edit.services.services')
                        @include('admin.quotations.edit.packages.packages')
                        @include('admin.quotations.edit.winch.winch_details')

                        @include('admin.quotations.total_details')
...
@section('modals')
    ...
    @include('admin.quotations.customers.form')

@endsection

@section('js-validation')

    {!! JsValidator::formRequest('App\Http\Requests\Admin\Quotation\UpdateQuotationRequest', '.form'); !!}
...
@section('js')
    ...
    <script type="application/javascript">
        ...
        function getPartsSection() {
            ...
            if ($("#spar_parts").is(":checked")) {
                ...
                if($("#ajax_parts_box").length){
                    ...
                    $.ajax({
                        url: "{{ route('admin:quotations.get.parts')}}",
        ...
        function partsByTypesFooter() {
            ...
            $.ajax({
                url: "{{ route('admin:quotations.parts.by.type.footer')}}",
        ...
        function getPartsDetails(id) {
            ...
            if (isExist) {
                ...
                $.ajax({
                    url: "{{ route('admin:quotations.parts.details') }}",
                    method: 'POST',
        ...
        function purchaseInvoiceData(part_id, index) {
            ...
            $.ajax({
                url: "{{ route('admin:quotations.get.purchase.invoice.data')}}",
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
        ...
        function getServicesSection() {
            ...
            if ($("#services_section_checkbox").is(":checked")) {
                ...
                if($("#ajax_service_box").length){
                    ...
                    $.ajax({
                        url: "{{ route('admin:quotations.get.services')}}",
        ...
        function servicesByTypesFooter() {
            ...
            $.ajax({
                url: "{{ route('admin:quotations.services.by.type.footer')}}",
        ...
        function getServiceById(service_type_id) {
            ...
            $.ajax({
                url: "{{ route('admin:quotations.get.services.by.type')}}",
        ...
        function getServiceDetails(service_id) {
            ...
            if (isExist) {
                ...
                $.ajax({
                    url: "{{ route('admin:quotations.get.services.details')}}",
        ...
        function getPackagesSection() {
            ...
            if ($("#package_section_checkbox").is(":checked")) {
                ...
                if($("#ajax_package_box").length){
                    ...
                    $.ajax({
                        url: "{{ route('admin:quotations.get.packages')}}",
        ...
        function getPackageDetails(package_id) {
            ...
            if (isExist) {
                ...
                $.ajax({
                    url: "{{ route('admin:quotations.package.details')}}",
        ...
        function store() {
            ...
            $.ajax({
                ...
                url: "{{ route('admin:quotations.update',$quotation->id)}}",
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                ...
                success: function (data) {
                    ...
                    swal({
                        text: "{{__('Quotation updated successfully!')}}",
                        icon: "success",
                    });
        ...
        function getWinchSection() {
            ...
            if (branch_id == "") {
                ...
                $("#winch_section_checkbox").prop('checked', false);
        ...
        function getWinchDistance () {
            ...
            $.ajax({
                url: "{{ route('admin:quotations.winch.get.distance')}}",
