...
@section('title')
<title>{{__('opening-balance.edit')}} </title>
@endsection
    ...
    <div class="row small-spacing">
        <nav>
            <ol class="breadcrumb" style="font-size: 37px; margin-bottom: 0px !important;padding:0px">
                ...
                <li class="breadcrumb-item">  {{__('opening-balance.edit')}} </li>
            </ol>
        </nav>
        <div class="col-xs-12">
            <div class=" card box-content-wg-new bordered-all primary">
                <h4 class="box-title with-control" style="text-align: initial"><i class="fa fa-gears"></i>  {{__('opening-balance.edit')}}
                ...
                <div class="box-content">
                    <form method="post" action="{{ route('opening-balance.update' ,['id' => $openingBalance->id]) }}" class="form"
                                        ...
                                        <div class="input-group">
                                            ...
                                            <select name="branch_id" class="form-control js-example-basic-single">
                                                ...
                                                @foreach(\App\Models\Branch::all() as $branch)
                                                    <option {{ $openingBalance->branch_id == $branch->id ? 'selected' : '' }}
                        ...
                        <div class="row">
                            <div class="form-group col-md-4">
                               ...
                               <span class="input-group-addon"><li class="fa fa-bars"></li></span>
                                <input class="form-control" readonly name="serial_number" value="{{ $openingBalance->serial_number }}"/>
                            </div>
                            </div>
                            ...
                            <div class="form-group col-md-4">
                                ...
                                <input class="form-control" type="date" name="operation_date" value="{{ $openingBalance->operation_date }}"/>
                            ...
                            <div class="form-group col-md-4">
                                ...
                                <input class="form-control" type="time" name="operation_time" value="{{ $openingBalance->operation_time }}"/>
...
<!-- 
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="text" name="user" class="form-control" disabled
                                               value="{{isset($openingBalance) ? optional($openingBalance->user)->name : auth()->user()->name}}">
                                    </div>
                        ...
                        @include('opening-balance.items-table' ,['balance_items' => $openingBalance->items])
                        <div class="row">
                            <div class="form-group col-sm-4">
                                <label> {{ __('opening-balance.total-money') }} </label>
                                <div class="input-group">
                                <span class="input-group-addon fa fa-money"></span>
                                <input class="form-control" readonly id="total-money" value="{{ $openingBalance->total_money }}"/>
                            </div>
                            </div>

                            <div class="form-group col-md-8">
                                <label> {{ __('opening-balance.notes') }} </label>
                                <textarea class="form-control" name="notes" rows="4">{{ $openingBalance->notes }}</textarea>
                            </div>
        ...
        function submit_form(event) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    window.location = '{{ route('opening-balance.edited') }}'
                },
                error: function (err) {
                    ...
                    if (err.status == 301) {
                        ...
                        if (err.responseJSON.message) alert(err.responseJSON.message ,callback)
                    }
