...
@section('title')
    <title>{{ __('Super Car') }} - {{ __('Edit Revenues Item') }} </title>
@endsection
...
@section('content')
    ...
    <nav>
            <ol class="breadcrumb" style="font-size: 37px; margin-bottom: 0px !important;padding:0px">
                ...
                <li class="breadcrumb-item active"> {{__('Edit Revenues Item')}}</li>
            </ol>
        </nav>
        <div class="col-xs-12">
        <div class=" card box-content-wg-new bordered-all primary">
                <h4 class="box-title with-control" style="text-align: initial"><i class="fa fa-money"></i>  {{__('Edit Revenues Item')}}
                ...
                <button class="control text-white" style="background:none;border:none;font-size:12px">{{__('Save')}}<img class="img-fluid" style="width:50px;height:50px;margin-top:-20px;margin-bottom:-13px" src="{{asset('assets/images/f1.png')}}"></button>
                    ...
                    <form  method="post" action="{{route('admin:revenues_Items.update', ['id' => $revenueItem->id])}}" class="form">
                        @csrf
                        @method('put')
                                ...
                                <div class="form-group has-feedback">
                                    ...
                                    <select name="branch_id" class="form-control  js-example-basic-single"
                                        ...
                                        @foreach(\App\Models\Branch::all() as $branch)
                                            <option
                                                value="{{$branch->id}}" {{$branch->id == $revenueItem->branch_id ? 'selected' : ''}}>{{$branch->name}}</option>
                                        @endforeach
                                    </select>
...
<div class="form-group has-feedback">
    ...
    @foreach(\App\Models\RevenueType::all() as $type)
            <option value="{{$type->id}}" {{ $type->id == $revenueItem->revenue_id ? 'selected' : '' }}>{{$type->type}}</option>
        @endforeach
    </select>
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" name="item_ar" class="form-control" id="inputNameAR"
                                       value="{{$revenueItem->item_ar}}" placeholder="{{__('Item in Arabic')}}">
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" name="item_en" class="form-control" id="item_en"
                                       value="{{$revenueItem->item_en}}" placeholder="{{__('Item in English')}}">
                            </div>
...
@section('js-validation')
    {!! JsValidator::formRequest('App\Http\Requests\Admin\RevenueItem\UpdateRevenueItemRequest', '.form'); !!}
