...
<nav>
    ...
    <li class="breadcrumb-item"><a href="{{route('admin:home')}}"> {{__('Dashboard')}}</a></li>
        ...
        <li class="breadcrumb-item active"> {{__('accounting-module.daily-restrictions-create')}}</li>
    </ol>
</nav>
...
<div class="col-xs-12 ui-sortable-handle">
				<div class="box-content card bordered-all primary">
                    ...
                    {{__('accounting-module.daily-restrictions-create')}}
        ...
        <form method="post" action="{{ route('daily-restrictions.store') }}" onsubmit="return check_fiscal_year()">
            @csrf
            ...
            <div class="form-group col-md-4">
                ...
                <input name="operation_number" class="form-control" onkeypress="return isNumber(event)"/>
            </div>
            <div class="form-group col-md-4">
                <label> {{ __('accounting-module.operation-date') }} </label>
                <input name="operation_date" type="date" class="form-control"/>
            </div>

            <div class="form-group col-md-4">
                <label> {{ __('accounting-module.debit-amount') }} </label>
                <input id="debit-amount" onkeydown="return false"
                    onkeyup="return false" onkeypress="return false"
                    name="debit_amount" class="form-control"/>
            </div>
            <div class="form-group col-md-4">
                <label> {{ __('accounting-module.credit-amount') }} </label>
                <input id="credit-amount" onkeydown="return false"
                    onkeyup="return false" onkeypress="return false"
                    name="credit_amount" class="form-control"/>
            </div>
            <div class="form-group col-md-4">
                <label> {{ __('accounting-module.records-number') }} </label>
                <input type="hidden" name="records_number" value="1"/>
                <input disabled id="records-number" class="form-control" value="1"/>
            </div>
            <div class="col-md-12">
                <table class="table table-striped table-bordered">
                    ...
                    <tbody id="table-body">
                        ...
                        <tr>
                            <td>
                                <input data-for="tree-code" type="hidden" name="table_data[{{ $randomId }}][account_tree_code]"/>
                                <input data-for="cost-code" type="hidden" name="table_data[{{ $randomId }}][cost_center_code]"/>
                                <select style="width:100%" name="table_data[{{ $randomId }}][accounts_tree_id]"
                                    ...
                                    @foreach($accounts_tree as $acc)
                                        <option value="{{ $acc->id }}" data-selected-id="{{ $acc->id }}"
                                            data-selected-code="{{ $acc->code }}">
                                            {{ $acc->code }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <td>
                                <input data-input-for="debit" class="form-control"
                                    ...
                                    name="table_data[{{ $randomId }}][debit_amount]" onchange="debit_n_credit_equal(event)">
                            </td>
                            <td>
                                <input data-input-for="credit" class="form-control"
                                    onkeypress="return isNumber(event)" value="0"
                                    name="table_data[{{ $randomId }}][credit_amount]" onchange="debit_n_credit_equal(event)">
                            </td>
                            <td>
                                <select class="form-control"
                                    name="table_data[{{ $randomId }}][cost_center_root_id]"
                                    onchange="load_cost_branches(event)">
                                    {!! \App\AccountingModule\Controllers\CostCenterCont::build_root_centers_options() !!}
                                </select>
                            </td>
                            <td>
                                <select class="form-control cost-centers-select" name="table_data[{{ $randomId }}][cost_center_id]">
                                    {!! \App\AccountingModule\Controllers\CostCenterCont::build_centers_options() !!}
                                </select>
                            </td>
                            <td>
                                <input class="form-control" name="table_data[{{ $randomId }}][notes]">
                            </td>
                            <td>
