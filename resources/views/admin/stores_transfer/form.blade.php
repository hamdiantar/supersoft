...
<div class="row top-data-wg" style="box-shadow: 0 0 7px 1px #DDD;margin:5px 5px 10px;padding-top:20px">
                ...
                <div class="form-group has-feedback">
                    ...
                    <div class="input-group">
                        ...
                        <select class="form-control js-example-basic-single" name="branch_id" id="branch_id"
                                onchange="changeBranch()" {{isset($storeTransfer) ? 'disabled':''}}>
                            <option value="">{{__('Select Branch')}}</option>

                            @foreach($branches as $branch)
                                <option
                                    value="{{$branch->id}}" {{isset($storeTransfer) && $storeTransfer->branch_id == $branch->id? 'selected':''}}
                    ...
                    @if(isset($storeTransfer))
                        <input type="hidden" name="branch_id" value="{{$storeTransfer->branch_id}}">
                    @endif
                </div>
            </div>
        </div>
    ...
    <div class="col-md-12">
            ...
            <div class="form-group">
                ...
                <div class="input-group">
                    ...
                    <input type="text" name="number" class="form-control" placeholder="{{__('Number')}}" disabled
                           value="{{old('number', isset($storeTransfer)? $storeTransfer->number : $number)}}">
                </div>
            ...
            <div class="form-group">
                ...
                <div class="input-group">
                    ...
                    <input type="date" name="date" class="form-control" id="date"
                           value="{{old('date', isset($storeTransfer)? $storeTransfer->transfer_date : \Carbon\Carbon::now()->format('Y-m-d'))}}">
                </div>
            ...
            <div class="form-group">
                ...
                <div class="input-group">
                    ...
                    <input type="time" name="time" class="form-control" id="time"
                           value="{{old('time', isset($storeTransfer)? $storeTransfer->time : \Carbon\Carbon::now()->format('H:i:s'))}}">
                </div>
        ...
        <div class="col-md-12">
        <div class="col-md-4">
            <div class="form-group" id="store_from">
                <label> {{ __('words.store-from') }} </label>
                <div class="input-group">
                <span class="input-group-addon fa fa-building-o"></span>
                <select class="form-control" name="store_from_id" id="store_from_select" onchange="load_my_parts()">
                    <option value=""> {{ __('Select One') }} </option>
                    @foreach($stores as $store)
                        <option value="{{ $store->id }}"
                            {{isset($storeTransfer) && $storeTransfer->store_from_id == $store->id ? 'selected':''}}>
                            {{ $store->name }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="col-md-4">
            <div class="form-group">
                ...
                <select class="form-control" name="store_to_id" id="store_to_select">
                    <option value=""> {{ __('Select One') }} </option>
                    @foreach($stores as $store)
                        <option value="{{ $store->id }}"
                            {{isset($storeTransfer) && $storeTransfer->store_to_id == $store->id ? 'selected':''}}>
                            {{ $store->name }}
                        </option>
                    @endforeach
                </select>
    ...
    @include('admin.stores_transfer.table_items')
...
<table class="table table-bordered">
  <tbody>
    ...
    <td style="background:#F9EFB7">
                <input type="text" disabled id="total_price"  style="background:#F9EFB7;border:none;text-align:center !important;" value="{{isset($storeTransfer) ? $storeTransfer->total : 0}}" class="form-control">
                </td>
  </tbody>
</table>
...
<table class="table table-bordered">
  <tbody>
    ...
    <td style="background:#D2F4F6">
                ...
                >{{old('description', isset($storeTransfer)? $storeTransfer->description :'')}}</textarea>
            </div>
