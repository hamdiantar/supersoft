...
@section('title')
    <title>{{ __('Super Car') }} - {{ __('words.bank-receives-edit') }} </title>
@endsection
        ...
        <nav>
            <ol class="breadcrumb" style="font-size: 37px; margin-bottom: 0px !important;padding:0px">
                ...
                <li class="breadcrumb-item">
                    <a href="{{route('admin:bank-receives.index')}}"> {{__('words.bank-receives')}}</a>
                </li>
                <li class="breadcrumb-item active"> {{__('words.bank-receives-edit')}}</li>
            </ol>
        </nav>
        <div class="col-xs-12">
            <div class=" card box-content-wg-new bordered-all primary">
                <h4 class="box-title with-control" style="text-align: initial">
                    <i class="fa fa-money"></i> {{ __('words.bank-receives-edit') }}
                ...
                <div class="box-content">
                    <form method="post" action="{{route('admin:bank-receives.update' ,['id' => $model->id])}}" class="form">
                        @csrf
                        @method('put')
                        <div class="row">
                            <div class="col-xs-12">
                                <div class="col-md-4">
                                    <div class="form-group has-feedback">
                                        ...
                                        <input type="text" readonly class="form-control"
                                            @if($model->source_type == 'bank')
                                                value="{{ optional($model->exchange_permission)->permission_number }}"
                                            @else
                                                value="{{ optional($model->locker_exchange_permission)->permission_number }}"
                                            @endif
                                            >
                                    </div>
                                </div>
                                <div class="col-md-3">
                                    <div class="form-group">
                                        ...
                                        <input type="text" readonly class="form-control"
                                            @if($model->source_type == 'bank')
                                                value="{{ optional($model->exchange_permission->fromBank)->name }}"
                                            @else
                                                value="{{ optional($model->locker_exchange_permission->fromLocker)->name }}"
                                            @endif
                                            >
                                    </div>
                                </div>
                                <div class="col-md-3">
                                    <div class="form-group">
                                        <label class="control-label">{{ __('words.bank-to') }}</label>
                                        <input type="text" readonly class="form-control"
                                            @if($model->source_type == 'bank')
                                                value="{{ optional($model->exchange_permission->toBank)->name }}"
                                            @else
                                                value="{{ optional($model->locker_exchange_permission->toBank)->name }}"
                                            @endif
                                            >
                                    </div>
                                </div>
                                <div class="col-md-2">
                                    <div class="form-group">
                                        ...
                                        <input type="text" id="type" readonly class="form-control"
                                            value="{{ __('words.'.$model->source_type) }}">
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-12">
...
@section('js-validation')
    {!! JsValidator::formRequest('App\Http\Requests\MoneyPermissions\BankReceiveEditRequest', '.form'); !!}
    ...
    <script type="application/javascript">
        $(document).on('change' ,'#exchange-permissions' ,function () {
            ...
            if (!exchange.amount) {
                ...
                $('#bank-from').val(exchange.from_bank.trans_name)
                $('#bank-to').val(exchange.to_bank.trans_name)
                $.ajax({
